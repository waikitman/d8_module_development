...
var pmGeneral = (function () {
    ...
    function miscHelpers() {
        ...
        $('.search-btn-wrap').hover(function () {
            $(this).parent('.search-wrapper').find('.search-box').toggleClass('btn-hover');
        });
        ...
        $('.empty-search').on('click', function () {
            var search = $(this).closest('.search-wrapper').find('.search-box');
            search.val('');
        ...
        $('.dismiss-alert').on('click', function (e) {
            $(this).parents('.alert-panel').fadeOut();
        });
    }
...
var pmSlideNavs = (function () {
    ...
    function mobileMenu() {
        ...
        if (pmGeneral.getCurrentBootstrapSize() != "xs" && pmGeneral.getCurrentBootstrapSize() != "sm") return false;
        ...
        $('body').on('click', 'a.menu-close, div.menu-blackout', function () {
            ...
            $('body, html').removeClass('chatlive-in');
            $("body").attr('style', '')
            $('.megamenu').find('.has-subnav').removeClass('subnav-in');
            $('.megamenu').removeClass('level-2-in');
        });

        $('body').on('click', '.has-subnav > a', function () {
            // if subnav is open
            if ($(this).parent().hasClass('subnav-in')) {
                // close subnav
                $(this).parent().removeClass('subnav-in');
                $(this).parents('.megamenu').removeClass('level-2-in');
            } else {
                // open subnav
                $(this).parents('.megamenu').addClass('level-2-in');
                $(this).parent().addClass('subnav-in');
                $(this).parent().siblings().removeClass('subnav-in');
                $('#menu').scrollTop(0);
            }
        });
        $('body').on('click', '.back', function () {
            // if subnav is open
            if ($(this).parents('.has-subnav').hasClass('subnav-in')) {
                // close subnav
                $(this).parents('.has-subnav').removeClass('subnav-in');
                $(this).parents('.megamenu').removeClass('level-2-in');
            }
        });
    }
    ...
    function chatLiveInit() {
        ...
        $('html.chatlive-in, body.chatlive-in').on('touchmove', function (e) {
            e.preventDefault();
        }, false);
...
var pmCarousels = (function () {
    ...
    function addCarousels() {
        ...
        $('.three-offers').slick({
            ...
            slidesToShow: 2,
            slidesToScroll: 1,
            prevArrow: '<button type="button" class="slick-arrow slick-prev"><i class="icon-arrow-2-left"></i></button>',
            nextArrow: '<button type="button" class="slick-arrow slick-next"><i class="icon-arrow-2-right"></i></button>',
    ...
    function addXsSmCarousels() {
        ...
        if (pmGeneral.getCurrentBootstrapSize() == "md" || pmGeneral.getCurrentBootstrapSize() == "lg") return false;
    ...
    function killXsSmCarousels() {
        if (!_carouselInitXSSM) return false;
        if (pmGeneral.getCurrentBootstrapSize() == "xs" || pmGeneral.getCurrentBootstrapSize() == "sm") return false;
        $('ul.compare-list.slick-initialized, .policies-container, .cover-level-container .policies').slick('unslick');
        _carouselInitXSSM = false;
    }
    ...
    function killXsCarousels() {
        if (!_carouselInitXS) return false;
        if (pmGeneral.getCurrentBootstrapSize() == "xs") return false;
        $('ul.slider-on-xs.slick-initialized, .features__grid.slick-initialized, .intro-feature__features--xs-carousel.slick-initialized').slick('unslick');
        _carouselInitXS = false;
    }
...
var pmMobileHelpers = (function () {
    ...
    function productsAccordion() {
        ...
        accItem.on('click', '.open-tile', function (e) {
            ...
            e.preventDefault();
            if ($(this).closest('.product-tile').hasClass('open')) {
                $(this).closest('.product-tile').removeClass('open').find('.show-content').slideUp('fast');
            } else {
                $(this).closest('.product-tile').addClass('open').find('.show-content').slideDown('fast');
                $(this).closest('.product-tile').siblings().removeClass('open').find('.show-content').slideUp('fast');
            }
        });
    }
    ...
    function contentTileAccordion() {
        ...
        $.each($accItem, function () {
            $(this).find('.tiles__item-content').hide();
        });
        $accItem.on('click', '.open-tile', function (e) {
            if ($('html').attr('data-bs-size') != 'xs') return;
            e.preventDefault();
            if ($(this).closest('.tiles__item').hasClass('open')) {
                $(this).closest('.tiles__item').removeClass('open').find('.tiles__item-content').slideUp('fast');
            } else {
                $(this).closest('.tiles__item').addClass('open').find('.tiles__item-content').slideDown('fast');
                $(this).closest('.tiles__item').siblings().removeClass('open').find('.tiles__item-content').slideUp('fast');
            }
        });
    }
    ...
    function footerAccordion() {
        ...
        accItem.on('click', 'header', function (e) {
            if (pmGeneral.getCurrentBootstrapSize() != "xs") return false;
            e.preventDefault();
            if ($(this).closest('.list-item').hasClass('open')) {
                $(this).closest('.list-item').removeClass('open').find('ul.list').slideUp('fast');
            } else {
                $(this).closest('.footer-lists').find('.list-item').removeClass('open').find('ul.list').slideUp('fast');
                $(this).closest('.list-item').addClass('open').find('ul.list').slideDown('fast');
            }
        });
    }

    function resetAccordions() {
        $('.footer-lists .list-item').removeClass('.open').find('ul.list').show();
        $('.tiles .product-tile, .tiles .tile__item-inner').removeClass('.open').find('.show-content').show();
    }

    function weatherAlert() {
        if (pmGeneral.getCurrentBootstrapSize() != "xs" && _weatherInit) return false;
        setTimeout(function () {
...
var pmModules = (function () {
    ...
    function parallaxScroll() {
        $(window).on('scroll', function () {
            $('.parallax-scroll').each(function (i, e) {
                if (pmGeneral.getCurrentBootstrapSize() !== "xs" && pmGeneral.isPartOnScreen(this)) {
    ...
    function showMore() {
        ...
        $('a.show-extra').on('click', function (e) {
            $('a.show-extra').hide();
            $('a.show-less').show();
            $('ul.checklist.reveal').show().each(function () {
                $(this).find('li').each(function (i) {
                    $(this).delay((i++) * 200).fadeTo(300, 1);
                });
            });
        ...
        $('a.show-less').on('click', function (e) {
            $('a.show-less').hide();
            $('a.show-extra').show();
            $('ul.checklist.reveal').slideUp(1000).find('li').fadeTo(1000, 0);
        });
    }
    ...
    function tabs() {
        ...
        $('a', $bigTabs).click(function () {
            ...
            $('section[data-big-tabs-id=' + $thisLi.data('big-tabs-show') + ']').find('.slick-initialized').slick('resize').slick('setPosition', 0);
